...
class AnimationPerformanceMonitor {
    ...
    const measureFrame = () => {
      ...
      if (frameCount % 60 === 0) {
        const averageFrameRate =
          frameRateSamples.reduce((sum, rate) => sum + rate, 0) /
          frameRateSamples.length;
  ...
  recordPerformanceIssue(type, details) {
    ...
    if (type === 'frame_drop') {
      const now = Date.now();
      if (
        !this.lastFrameDropWarning ||
        now - this.lastFrameDropWarning > 5000
      ) {
  ...
  generateOptimizationRecommendations(animationData) {
    ...
    const concurrentAnimations = Array.from(
      this.metrics.animations.values()
  ...
  getPerformanceSummary() {
    const completedAnimations = Array.from(
      this.metrics.animations.values()
    ).filter(anim => anim.status === 'completed');

    const averageDuration =
      completedAnimations.length > 0
        ? completedAnimations.reduce((sum, anim) => sum + anim.duration, 0) /
          completedAnimations.length
        : 0;

    const recentFrameRates = this.metrics.frameRates.slice(-10);
    const averageFrameRate =
      recentFrameRates.length > 0
        ? recentFrameRates.reduce(
            (sum, rate) => sum + parseFloat(rate.averageFrameRate),
            0
    ...
    const averageMemoryUsage =
      recentMemoryUsage.length > 0
        ? recentMemoryUsage.reduce((sum, mem) => sum + mem.usedJSHeapSize, 0) /
