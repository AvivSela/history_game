...
describe('gameLogic', () => {
  ...
  describe('isTimelineChronological', () => {
    it('should return true for chronologically ordered timeline', () => {
      const chronologicalTimeline = [mockEvents[0], mockEvents[1], mockEvents[2]]
      
      expect(isTimelineChronological(chronologicalTimeline)).toBe(true)
    })

    it('should return false for non-chronological timeline', () => {
      const nonChronologicalTimeline = [mockEvents[1], mockEvents[0], mockEvents[2]]
  ...
  describe('generatePlacementFeedback', () => {
    it('should generate positive feedback for correct placement', () => {
      const feedback = generatePlacementFeedback(testCard, [mockEvents[0]], 1, 1, true)
    ...
    it('should generate directional feedback for incorrect placement', () => {
      const feedback = generatePlacementFeedback(testCard, [mockEvents[0]], 0, 1, false)
  ...
  describe('generateHint', () => {
    it('should provide decade hint for empty timeline', () => {
      const hint = generateHint(testCard, [])
      
      expect(hint).toContain('1980s')
      expect(hint).toContain('💡')
    })

    it('should provide "before" hint when card is older than timeline', () => {
      const oldCard = { dateOccurred: '1900-01-01T00:00:00.000Z' }
      const hint = generateHint(oldCard, [mockEvents[0], mockEvents[1]])
      
      expect(hint).toContain('1900')
      expect(hint).toContain('before all events')
    })

    it('should provide "after" hint when card is newer than timeline', () => {
      const newCard = { dateOccurred: '2000-01-01T00:00:00.000Z' }
      const hint = generateHint(newCard, [mockEvents[0], mockEvents[1]])
      
      expect(hint).toContain('2000')
      expect(hint).toContain('after all events')
    })
  })
  ...
  describe('createGameSession', () => {
    const sixEvents = [
      { id: 1, title: 'Event 1', dateOccurred: '2000-01-01', category: 'A', difficulty: 1 },
      { id: 2, title: 'Event 2', dateOccurred: '2001-01-01', category: 'A', difficulty: 1 },
      { id: 3, title: 'Event 3', dateOccurred: '2002-01-01', category: 'A', difficulty: 1 },
      { id: 4, title: 'Event 4', dateOccurred: '2003-01-01', category: 'A', difficulty: 1 },
      { id: 5, title: 'Event 5', dateOccurred: '2004-01-01', category: 'A', difficulty: 1 },
      { id: 6, title: 'Event 6', dateOccurred: '2005-01-01', category: 'A', difficulty: 1 },
    ];
