...
describe('DifficultySelector', () => {
  ...
  describe('Rendering', () => {
    ...
    test('renders difficulty descriptions', () => {
      ...
      expect(screen.getByText(/Relaxed gameplay with generous time limits/)).toBeInTheDocument();
      expect(screen.getByText(/Balanced challenge with moderate time pressure/)).toBeInTheDocument();
      expect(screen.getByText(/Challenging gameplay with strict time limits/)).toBeInTheDocument();
      expect(screen.getByText(/Maximum challenge with minimal assistance/)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    ...
    test('has proper ARIA describedby attributes', () => {
      ...
      expect(easyRadio).toHaveAttribute('aria-describedby', 'difficulty-easy-description');
      expect(easyDescription).toHaveAttribute('id', 'difficulty-easy-description');
    });
  ...
  describe('User Interactions', () => {
    ...
    test('updates selection when value prop changes', () => {
      ...
      // Change to Hard
      rerender(<DifficultySelector {...defaultProps} value={DIFFICULTY_LEVELS.HARD} />);
    ...
    test('applies selected styling to current option', () => {
      ...
      const mediumOption = screen.getByDisplayValue('medium').closest('.difficulty-selector__option');
  ...
  describe('Keyboard Navigation', () => {
    ...
    test('wraps around to last option when pressing ArrowLeft on first option', () => {
      render(<DifficultySelector {...defaultProps} value={DIFFICULTY_LEVELS.EASY} />);
      ...
      fireEvent.keyDown(fieldset, { key: 'ArrowLeft' });
      expect(defaultProps.onChange).toHaveBeenCalledWith(DIFFICULTY_LEVELS.EXPERT);
    });
  ...
  describe('CSS Classes', () => {
    ...
    test('does not apply selected class to unselected options', () => {
      ...
      const easyOption = screen.getByDisplayValue('easy').closest('.difficulty-selector__option');
      const hardOption = screen.getByDisplayValue('hard').closest('.difficulty-selector__option');
      
      expect(easyOption).not.toHaveClass('difficulty-selector__option--selected');
      expect(hardOption).not.toHaveClass('difficulty-selector__option--selected');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    test('handles null onChange prop', () => {
      ...
      expect(() => {
        render(<DifficultySelector value={DIFFICULTY_LEVELS.MEDIUM} onChange={null} />);
      }).not.toThrow();
    });
  });
