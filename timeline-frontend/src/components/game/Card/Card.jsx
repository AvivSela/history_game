    ...
    const renderPlayerHandContent = () => (
      <>
        <div className="bg-gray-50 p-2 flex justify-between items-center text-gray-800 flex-shrink-0 border-b border-gray-200">
          <div className="flex gap-1 items-center">
            {Array.from({ length: event.difficulty }, (_, i) => (
              <span key={i} className="text-lg text-star drop-shadow-sm">
                ★
              </span>
            ))}
          </div>
          <div
            className={`flex items-center gap-1.5 bg-black/5 rounded-xl px-2 py-1 border border-gray-200 ${getCategoryClass(event.category)}`}
          >
            <div className="text-sm">{getCategoryIcon(event.category)}</div>
            <div className="text-[10px] uppercase tracking-wider font-bold text-gray-800 drop-shadow-sm">
              {event.category}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-t border-gray-200 px-3 py-4 flex-grow flex flex-col">
          <div className="font-card text-xs font-bold text-gray-800 uppercase tracking-wider text-center mb-2.5 drop-shadow-sm">
            Event Description
          </div>
          <div className="font-card text-xs text-gray-800 leading-relaxed text-justify flex-grow overflow-hidden">
            <div className="line-clamp-3">
              {event.description ||
                'A significant historical event that shaped the course of history.'}
            </div>
          </div>
        </div>
      </>
