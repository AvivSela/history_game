...
describe('Wrong Placement Animation', () => {
  ...
  const MockGameComponent = () => {
    ...
    const [selectedCard] = useState({
      ...
      description:
        'The Berlin Wall is opened, allowing free travel between East and West Berlin',
      category: 'Political',
      difficulty: 2,
    });
    ...
    return (
      <div>
        <button
          onClick={handleWrongPlacement}
          data-testid="trigger-wrong-placement"
        >
  ...
  it('should add animation class to timeline element when wrong placement is triggered', async () => {
    ...
    await waitFor(() => {
      const timelineContent = screen.getByTestId('timeline-content');
      const hasClass = Array.from(timelineContent.classList).some(cls =>
        cls.includes('wrong-placement')
      );
